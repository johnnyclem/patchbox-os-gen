...
from gui.screens.arp import ArpScreen
from gui.screens.fx import FxScreen
from gui.screens.matrix import MatrixScreen
from gui.screens.perform import PerformScreen
from gui.screens.settings import SettingsScreen

log = logging.getLogger("midiranger.gui")

SCREENS = (PerformScreen, MatrixScreen, FxScreen, ArpScreen, SettingsScreen)
MESSAGE_MS = 2500
...
class App:
    ...
    def __init__(self, engine, size=(theme.WIDTH, theme.HEIGHT),
                 ...
                 project_path: Path | None = None, pots=None) -> None:
        self.engine = engine
        self.config = config
    ...
    def new_project(self) -> None:
        ...
        self.engine.submit(cmd.ClearRoutes())
        for route in project.params.get("routes", []):
            from rangerkit.routing import Route
            self.engine.submit(cmd.ToggleRoute(Route(
    ...
    def _key(self, event) -> bool:
        ...
        if event.key == pygame.K_ESCAPE:
            ...
            self.engine.submit(cmd.SetBypass(on=not self.engine.bypass))
        elif pygame.K_1 <= event.key <= pygame.K_8:
            self.engine.submit(cmd.RecallScene(slot=event.key - pygame.K_1))
        else:
            return False
        return True
    ...
    # --- drawing -------------------------------------------------------------
    def _draw(self, snapshot: MrSnapshot) -> None:
    ...
    def _draw_transport(self, snapshot: MrSnapshot) -> None:
        ...
        button(self.surface, self.chrome, "play", cells[2],
               ...
               sub="arps + lfos")
        button(self.surface, self.chrome, "bypass", cells[3], "BYP", 15,
               active=snapshot.bypass, color=theme.DANGER,
               sub="rack off" if snapshot.bypass else "rack on")
