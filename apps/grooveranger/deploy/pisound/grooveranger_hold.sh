#!/bin/sh
# GrooveRanger — hold actions. Map this to HOLD_1S/3S/5S/HOLD_OTHER in
...
/usr/local/bin/grooveranger-btn HOLD "${CLICKS}" "${SECONDS_HELD}"
status=$?
if [ "${status}" -eq 2 ]; then
    command -v flash_leds >/dev/null 2>&1 && flash_leds 200
    command -v log >/dev/null 2>&1 &&
        log "grooveranger: no instrument on the button socket"
    if [ "${SECONDS_HELD}" -ge 7 ] 2>/dev/null; then
        command -v log >/dev/null 2>&1 &&
            log "grooveranger: no instrument, ${SECONDS_HELD}s hold — shutting down"
        shutdown -h now
    fi
fi
exit "${status}"
