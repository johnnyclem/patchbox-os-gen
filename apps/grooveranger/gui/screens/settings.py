...
# The kit's destination is bindable, plus every input a pad controller
# might arrive on. ``internal`` needs no port.
...
class SettingsScreen(Screen):
    ...
    def _titled(self, surface, rect, label):
        section_head(surface, pygame.Rect(rect.x, rect.y, rect.width, 18),
                     label)
        return pygame.Rect(rect.x, rect.y + 20, rect.width,
                           rect.height - 20)
    ...
    def _ports(self, surface, rect, s) -> None:
        ...
        lcd(surface, cells[-1], s.backend, size=14, label="MIDI + AUDIO")
    ...
    def _rig(self, surface, rect, s) -> None:
        ...
        lcd(surface, cells[3], f"{s.voices}", size=18,
            label="NOTES SOUNDING")
