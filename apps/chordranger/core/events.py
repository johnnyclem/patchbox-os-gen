"""Re-export shim — the tick/event vocabulary now lives in rangerkit.

ChordRanger was the donor: this module's contents moved into
``rangerkit.events`` when the suite grew siblings (RANGER-SUITE-PLAN
Phase 0) and came back home as a shim in Phase 7. Replacing the module in
``sys.modules`` aliases it completely — every ``from core.events import X``
...
try:
    from rangerkit import events as _shared
