"""Re-export shim — the tick clocks now lives in rangerkit.

ChordRanger was the donor: this module's contents moved into
``rangerkit.clock`` when the suite grew siblings (RANGER-SUITE-PLAN
Phase 0) and came back home as a shim in Phase 7. Replacing the module in
``sys.modules`` aliases it completely — every ``from core.clock import X``
...
try:
    from rangerkit import clock as _shared
