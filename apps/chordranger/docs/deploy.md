...
`CHORDRANGER_WIDTH`/`HEIGHT` wins, else HyperPixel (`ENABLE_HYPERPIXEL4=1` →
800×480), else Waveshare DPI (`ENABLE_WAVESHARE_DPI=1` → 640×480), else the
HDMI bar (1280×400). The panel picks its own chrome from those two numbers —
side rails at 2:1 or wider, a stacked top band and bottom tabs otherwise — so
