"""Re-export shim — the widget kit now lives in rangerkit.

ChordRanger was the donor: this module's contents moved into
``rangerkit.gui.widgets`` when the suite grew siblings (RANGER-SUITE-PLAN
Phase 0) and came back home as a shim in Phase 7. Replacing the module in
``sys.modules`` aliases it completely — every ``from gui.widgets import X``
...
try:
    from rangerkit.gui import widgets as _shared
