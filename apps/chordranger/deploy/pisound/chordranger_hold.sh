#!/bin/sh
# ChordRanger — hold actions. Map this to HOLD_1S/3S/5S/HOLD_OTHER in
...
/usr/local/bin/chordranger-btn HOLD "${CLICKS}" "${SECONDS_HELD}"
status=$?
if [ "${status}" -eq 2 ]; then
    ...
    if [ "${SECONDS_HELD}" -ge 7 ] 2>/dev/null; then
        command -v log >/dev/null 2>&1 &&
            log "chordranger: no instrument, ${SECONDS_HELD}s hold — shutting down"
        shutdown -h now
    fi
fi
exit "${status}"
