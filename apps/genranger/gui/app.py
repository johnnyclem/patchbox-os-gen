...
from gui.screens.layers import LayersScreen
from gui.screens.map import MapScreen
from gui.screens.perform import PerformScreen
from gui.screens.seeds import SeedsScreen
from gui.screens.settings import SettingsScreen

log = logging.getLogger("genranger.gui")

SCREENS = (PerformScreen, MapScreen, LayersScreen, SeedsScreen,
           SettingsScreen)
MESSAGE_MS = 2500
...
class App:
    ...
    def _key(self, event) -> bool:
        ...
        if event.key == pygame.K_ESCAPE:
            ...
            self.engine.submit(cmd.SetCruise(on=not self.engine.cruise.on))
        elif event.key == pygame.K_m:
            self.engine.submit(cmd.MutateNow())
        elif event.key == pygame.K_l:
            self.engine.submit(cmd.LockAll())
        elif pygame.K_1 <= event.key <= pygame.K_8:
            self.engine.submit(cmd.RecallSeed(slot=event.key - pygame.K_1))
        else:
            return False
        return True
    ...
    # --- drawing -------------------------------------------------------------
    def _draw(self, snapshot: GvSnapshot) -> None:
    ...
    def _draw_transport(self, snapshot: GvSnapshot) -> None:
        ...
        button(self.surface, self.chrome, "play", cells[2],
               ...
               sub="the piece")
        button(self.surface, self.chrome, "cruise", cells[3], "CRZ", 15,
               active=snapshot.cruise_on, color=theme.ACCENT2,
               sub="evolving" if snapshot.cruise_on else "held")
