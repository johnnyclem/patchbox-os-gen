"""GenRanger entry point.
    ...
    python main.py --config /etc/genranger/config.toml       appliance

Both paths build the same rig: engine + multi-channel MIDI out + the PiSound
button bridge + the pots service — and both press play, because a generative
box that boots silent fails its own first metric. The GUI is only a client;
closing the window stops the picture, not the piece.
"""
...
log = logging.getLogger("genranger.main")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="GenRanger — generative sequencer for Patchbox OS")
    ...
    parser.add_argument("--project", type=Path, default=None,
                        help=".gvproj to load at start")
    parser.add_argument("--config", type=Path, default=None,
                        help="config.toml (appliance: "
                             "/etc/genranger/config.toml)")
...
def build_rig(project_path: Path | None, config):
    ...
    def on_input(endpoint, event, ts):
        # The pots get first crack at CC traffic; everything else is ignored
        # — GenRanger generates, it does not process input notes.
        if pots_ref and event.kind is EventKind.CC:
            pots_ref[0].on_cc(event)
    ...
    # Bind every destination the layers name before the first tick: a rig
    # that comes up unbound is silent until somebody visits SET, and on an
    # appliance with no keyboard that is a support call.
    destinations = {layer.params.dest for layer in engine.layers
                    if layer.params.enabled}
    routing.autobind(midi, destinations)
    ...
    if config.button.enabled:
        ...
        from core.commands import LockAll, MutateNow
        actions = {
            "play_stop": lambda: engine.submit(enginebase.TogglePlay()),
            "mutate_now": lambda: engine.submit(MutateNow()),
            "lock_all": lambda: engine.submit(LockAll()),
            "panic": lambda: engine.submit(enginebase.Panic()),
            "save_project": lambda: _headless_save(engine, config),
        }
        defaults = {"CLICK_1": "play_stop", "CLICK_2": "mutate_now",
                    "CLICK_3": "lock_all", "HOLD_1S": "save_project",
                    "HOLD_5S": "panic"}
        button = ButtonServer(actions, socket_path(config),
                              raw_map=getattr(config.button, "map", {}),
                              defaults=defaults, thread_name="gv-button")
        if not button.start():
            button = None
...
def run_deck(args: argparse.Namespace, config) -> int:
    ...
    button and pots stay up between shows, so the piece plays straight
    through a trip back to the launcher.
    """
    ...
    try:
        return run_deck_session(make_app, args.deck_socket, "genranger")
    finally:
        shutdown_rig(engine, midi, button, pots)
