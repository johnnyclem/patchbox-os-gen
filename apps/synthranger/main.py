"""GrooveRanger entry point.
    ...
    python main.py --config /etc/synthranger/config.toml      appliance

Both paths build the same rig: engine + multi-port MIDI with the synth on
...
log = logging.getLogger("synthranger.main")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SynthRanger — multi-engine polysynth for Patchbox OS")
    ...
    parser.add_argument("--project", type=Path, default=None,
                        help=".syproj to load at start")
    parser.add_argument("--config", type=Path, default=None,
                        help="config.toml (appliance: "
                             "/etc/synthranger/config.toml)")
...
def build_rig(project_path: Path | None, config):
    """Engine + MIDI + synth-on-internal + audio + button + pots."""
    ...
    def on_input(endpoint, event, ts):
        # The pots get first crack at CC traffic; notes play the parts.
    ...
    # The synth answers on the internal endpoint; the release book
    # releases its voices through the very same note-offs external gear
    # gets. Preset parsing stays off the tick thread.
    synth = Synth()
    midi = SynthMidiBridge(midi, synth)
    engine = SynthRangerEngine(project, midi, RealClock(), config=config)
    engine_ref.append(engine)
    synth.set_parts(engine.parts)
    synth.set_tempo(engine.bpm)
    audio = open_audio(synth, config=config)
    synth._audio_out = audio
    ...
    if config.button.enabled:
        ...
        from core.commands import SelectPart
        from core.parts import PARTS

        def next_part():
            snapshot = engine.snapshot()
            engine.submit(SelectPart(
                part=(snapshot.selected_part + 1) % PARTS))

        actions = {
            "play_stop": lambda: engine.submit(enginebase.TogglePlay()),
            "next_part": next_part,
            "all_off": lambda: engine.submit(enginebase.Panic()),
            "panic": lambda: engine.submit(enginebase.Panic()),
            "save_project": lambda: _headless_save(engine, config),
        }
        defaults = {"CLICK_1": "play_stop", "CLICK_2": "next_part",
                    "CLICK_3": "all_off", "HOLD_1S": "save_project",
                    "HOLD_5S": "panic"}
        button = ButtonServer(actions, socket_path(config),
                              raw_map=getattr(config.button, "map", {}),
                              defaults=defaults, thread_name="sy-button")
        if not button.start():
            button = None
    return engine, midi, button, pots, synth
...
def shutdown_rig(engine, midi, button=None, pots=None, synth=None) -> None:
    if button is not None:
        button.stop()
    if pots is not None:
        pots.stop()
    engine.shutdown()
    audio = getattr(synth, "_audio_out", None)
...
def run_headless(project_path: Path | None, config) -> int:
    ...
    engine, midi, button, pots, synth = build_rig(project_path, config)
    print(f"synthranger: headless, backend="
          f"{getattr(midi, 'backend_name', 'null')} — Ctrl-C to stop")
    received = signal.sigwait([signal.SIGINT, signal.SIGTERM])
    print(f"synthranger: signal {received}, shutting down")
    shutdown_rig(engine, midi, button, pots, synth)
    return 0
...
def run_gui(args: argparse.Namespace, config) -> int:
    ...
    engine, midi, button, pots, synth = build_rig(args.project, config)
    try:
...
def run_deck(args: argparse.Namespace, config) -> int:
    ...
    synth, audio, button and pots stay up between shows, so held voices and
    the transport play straight through a trip back to the launcher.
    """
    ...
    try:
        return run_deck_session(make_app, args.deck_socket, "synthranger")
    finally:
        shutdown_rig(engine, midi, button, pots, synth)
