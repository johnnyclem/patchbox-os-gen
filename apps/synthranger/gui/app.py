...
Two SynthRanger-specific duties live here because they are file and
...
from core.preset import (list_presets, load_preset, save_preset,
                         user_dir)
from core.project import EXTENSION
from core.version import APP_NAME, __version__
from gui.screens.browser import BrowserScreen
from gui.screens.edit import EditScreen
from gui.screens.mix import MixScreen
from gui.screens.perform import PerformScreen
from gui.screens.settings import SettingsScreen

log = logging.getLogger("synthranger.gui")

SCREENS = (PerformScreen, EditScreen, BrowserScreen, MixScreen,
           SettingsScreen)
MESSAGE_MS = 2500
...
class App:
    ...
    def __init__(self, engine, size=(theme.WIDTH, theme.HEIGHT),
                 ...
                 synth=None, deck: bool = False) -> None:
        self.engine = engine
        self.config = config
    ...
    # --- presets --------------------------------------------------------------
    def presets(self) -> tuple:
        return self._presets

    def refresh_presets(self) -> None:
        self._presets = tuple(list_presets(self.config))

    def load_preset_index(self, index: int, slot_b: bool) -> None:
        """Parse on this thread; the engine gets a dict, the synth gets
        the swap via parts_rev."""
        if not 0 <= index < len(self._presets):
            return
        path = self._presets[index]
        try:
            patch = load_preset(path)
        except (OSError, ValueError) as exc:
            log.warning("preset %s unreadable: %s", path.name, exc)
            self.message(f"BAD PRESET {path.stem.upper()[:10]}")
            return
        self.engine.submit(cmd.SetPatchState(
            part=self.engine.selected_part, params=patch.to_config(),
            slot_b=slot_b))

    def save_preset(self) -> None:
        part = self.engine.parts[self.engine.selected_part]
        directory = user_dir(self.config)
        if directory is None:
            directory = Path("data") / "user-presets"
        try:
            path = save_preset(directory, part.patch)
        except OSError as exc:
            log.warning("preset save failed: %s", exc)
            self.message("SAVE FAILED")
            return
        self.refresh_presets()
        self.message(f"SAVED {path.stem.upper()[:12]}")

    def _sync_synth(self, snapshot: SySnapshot) -> None:
        if self.synth is None:
            return
        self.synth.set_tempo(snapshot.bpm)
        if snapshot.parts_rev != self._parts_rev:
            self._parts_rev = snapshot.parts_rev
            # engine.parts is an immutable tuple; reading the reference
            # from this thread is safe.
            self.synth.set_parts(self.engine.parts)
    ...
    def _key(self, event) -> bool:
        ...
        if event.key == pygame.K_ESCAPE:
            ...
            self.engine.submit(cmd.SelectPart(
                part=event.key - pygame.K_1))
        else:
            return False
        return True
    ...
    def _chrome_event(self, event) -> bool:
        ...
        if key.startswith("tab"):
            ...
            snapshot = self.engine.snapshot()
            self.engine.submit(cmd.SelectPart(
                part=(snapshot.selected_part + 1) % 4))
        elif key == "bpm-":
    ...
    # --- drawing -------------------------------------------------------------
    def _draw(self, snapshot: SySnapshot) -> None:
    ...
    def _draw_transport(self, snapshot: SySnapshot) -> None:
