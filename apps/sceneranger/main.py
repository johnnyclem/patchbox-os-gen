"""SceneRanger entry point.
    ...
    python main.py --config /etc/sceneranger/config.toml       appliance

Both paths build the same rig: engine + multi-port MIDI + the PiSound
button bridge + the pots service. The transport arms on the first launch —
a session view starts when you touch a clip. The GUI is only a client;
closing the window stops the picture, not the session.
"""
...
log = logging.getLogger("sceneranger.main")


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SceneRanger — session clip launcher for Patchbox OS")
    ...
    parser.add_argument("--project", type=Path, default=None,
                        help=".scproj to load at start")
    parser.add_argument("--config", type=Path, default=None,
                        help="config.toml (appliance: "
                             "/etc/sceneranger/config.toml)")
...
def build_rig(project_path: Path | None, config):
    ...
    engine = SceneRangerEngine(project, midi, RealClock(), config=config)
    engine_ref.append(engine)

    # Bind every destination the tracks name — and the inputs, because the
    # slot recorder needs something to record from. A rig that comes up
    # unbound is silent until somebody visits LIBRARY, and on an appliance
    # with no keyboard that is a support call.
    endpoints = {params.dest for params in engine.grid.tracks}
    ...
    if config.button.enabled:
        ...
        from core.commands import Disarm, LaunchScene, StopAll
        state = {"scene": -1}

        def next_scene():
            filled = [index for index, used in
                      enumerate(engine.grid.filled_scenes()) if used]
            if not filled:
                return
            later = [index for index in filled if index > state["scene"]]
            state["scene"] = (later or filled)[0]
            engine.submit(LaunchScene(scene=state["scene"]))

        actions = {
            "play_stop": lambda: engine.submit(enginebase.TogglePlay()),
            "scene_fire": next_scene,
            "record_arm": lambda: engine.submit(Disarm()),
            "stop_all": lambda: engine.submit(StopAll()),
            "panic": lambda: engine.submit(enginebase.Panic()),
            "save_project": lambda: _headless_save(engine, config),
        }
        defaults = {"CLICK_1": "play_stop", "CLICK_2": "scene_fire",
                    "CLICK_3": "stop_all", "HOLD_1S": "save_project",
                    "HOLD_5S": "panic"}
        button = ButtonServer(actions, socket_path(config),
                              raw_map=getattr(config.button, "map", {}),
                              defaults=defaults, thread_name="sc-button")
        if not button.start():
            button = None
...
def run_deck(args: argparse.Namespace, config) -> int:
    ...
    button and pots stay up between shows, so clips and the transport play
    ...
    try:
        return run_deck_session(make_app, args.deck_socket, "sceneranger")
    finally:
        shutdown_rig(engine, midi, button, pots)
