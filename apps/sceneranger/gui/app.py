...
from gui.screens.arrange import ArrangeScreen
...
log = logging.getLogger("sceneranger.gui")

SCREENS = (PerformScreen, RoutingScreen, ArrangeScreen,
           LibraryScreen)
MESSAGE_MS = 2500
...
class App:
    ...
    def _key(self, event) -> bool:
        ...
        if event.key == pygame.K_ESCAPE:
            ...
            self.engine.submit(cmd.StopAll())
        elif pygame.K_1 <= event.key <= pygame.K_8:
            self.engine.submit(cmd.LaunchScene(
                scene=event.key - pygame.K_1))
        else:
            return False
        return True
    ...
    # --- drawing -------------------------------------------------------------
    def _draw(self, snapshot: ScSnapshot) -> None:
    ...
    def _draw_transport(self, snapshot: ScSnapshot) -> None:
        ...
        button(self.surface, self.chrome, "play", cells[2],
               ...
               sub=f"q: {snapshot.quantize}")
        button(self.surface, self.chrome, "stopall", cells[3], "STOP", 14,
               active=bool(snapshot.armed), color=theme.ACCENT2,
               sub="all tracks")
